...
func grinHandler(w http.ResponseWriter, r *http.Request) {
	fmt.Fprintf(w, "😀")
}
...
func main() {
    ...
    // When we receive a request at '/grin' or '/grinning', we want
    // the `grinHandler` function to handle it.
	http.HandleFunc("/grin", grinHandler)
	http.HandleFunc("/grinning", grinHandler)
